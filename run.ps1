...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Title 'LiveKit Server'; docker run --rm -p 7880:7880 -p 7881:7881 -p 7882:7882/udp livekit/livekit-server --dev --keys `"devkey: secret`" --bind 0.0.0.0"
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Title 'FastAPI Backend'; .\.venv\Scripts\activate.ps1; python -m uvicorn backend.main:app --host 0.0.0.0 --port 8000 --reload"
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Title 'Next.js Frontend'; cd frontend; npm run dev -- -p 3000"
...
if ($hasLiveKit) {
    ...
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "Title 'LiveKit Voice Agent'; .\.venv\Scripts\activate.ps1; python -m backend.livekit_agent dev"
} else {
